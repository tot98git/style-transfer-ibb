...
def process_warp(src_img, result_img, tri_affines, dst_points, delaunay,):
    ...
    for simplex_index in range(len(delaunay.simplices)):
        ...
        x, y = coords.T
        interpolation = bilinear_interpolate(src_img, out_coords)

        try:
            result_img[y, x] = interpolation
        except:
            print("It broke down but it's okay")
...
def warp_image(src_img, src_points, dest_points, dest_shape=(250, 250), dtype=np.uint8):
    ...
    result_img = process_warp(
        src_img, result_img, tri_affines, dest_points, delaunay)
