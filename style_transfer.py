...
class StyleTransfer:
    def main(self):
        example_src = "poses/woman_smiling_nm.jpeg"
        ...
        f1_f = f1.resize_img((500, 500), f1_f)

        f2_f = f2.process_face()
        f2_f = f2.resize_img((500, 500), f2_f)
        ...
        f1_L, f1_A, f1_B = f1.decompose(face1)
        f2_L, f2_A, f2_B = f2.decompose(warped_img)
        ...
        finalA = mask_transformer(alpha_blend, f1_A, f2_A, .7)
        finalB = mask_transformer(alpha_blend, f1_B, f2_B, .7)
        detail = mask_transformer(
            skin_detail_transfer, f1_detail, f2_detail, .6, .3,)
        ...
        cv2.imwrite('tmp/test.png', transformed)
